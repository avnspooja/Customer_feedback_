...
def load_keywords():
    try:
        with open("Milestone3_Keyword_Insights.csv", "r", encoding="utf-8") as f:content = f.read()
...
if not filtered_df.empty:
    ...
    bars = ax1.bar(counts.index, counts.values, color=[colors.get(s, 'gray')
...
if not filtered_df.empty:
    ...
    product_sent['Positive %'] = (product_sent.get('Positive', 0) /
    product_sent['Total'] * 100).round(1)
    product_sent = product_sent.sort_values('Positive %', ascending=False)
    st.dataframe(product_sent.style.format(precision=1),
...
if not filtered_df.empty:
    ...
    ax_hist.hist(filtered_df["confidence_score"], bins=25,
